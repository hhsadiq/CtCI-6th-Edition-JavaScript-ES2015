 ...
 * length of the slice. To be precise, the average equals (A[P] + A[P + 1] +
 * ... + A[Q]) / (Q − P + 1).
...
export function solution(A) {
  ...
  A.forEach((value, i) => {
    if (i + 1 < A.length) {
      const sliceOfTwo = (A[i] + A[i + 1]) / 2;
      if (sliceOfTwo < minSum) {
        desiredIndex = i;
        minSum = sliceOfTwo;
      }
    }
    if (i + 2 < A.length) {
      const sliceOfThree = (A[i] + A[i + 1] + A[i + 2]) / 3;
      if (sliceOfThree < minSum) {
        desiredIndex = i;
        minSum = sliceOfThree;
      }
    }
